...
class Message:
    ...
    def createMessage(self, **kwargs):
        ...
        return msg.attach(MIMEText(self.template.render(kwargs)), 'plain')

def sendEmail(emailTemp, id, link, previousPrice, newPrice, country):
    ...
    print(testEmail)
    server = EmailServer('smtp.google.com', 587, testEmail, os.environ['PASSWORD'])
    Subject = "The item with ID {} just dropped its price!!".format(id)
    mess = Message(emailTemp, testEmail, testEmail, Subject)\
        .createMessage(id=id, link=link, previousPrice=previousPrice, newPrice=newPrice, country=country)
    server.sendMessage(mess)
    server.quit()

if __name__ == "__main__":
    sendEmail("emailTef_rd_m=A3P5ROKL5A1OLE&pf_rd_r=NVB80W7WF284EWYCFR5mplate.html", "TESTE", "https://www.amazon.co.uk/dp/B07WLT8RLB/", "90", "70", "UK")